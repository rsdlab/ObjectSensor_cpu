...
//  /* set up work arrays */
//  e=work;
//  hx=e + n;			/* measurement vector of new parameters 'p[]' */
//  jacTe=hx + n;
//  jac=jacTe + m;
//  jacTjac=jac + nm;
//  Dp=jacTjac + m*m;
...
//  (*func)(p, hx, m, n, adata); nfev=1;
//  for(i=0, p_eL2=0.0; i<n; ++i){
//    e[i]=tmp=x[i]-hx[i];	/* 'x[]' is the measurement vector to be compared with */
//    p_eL2+=tmp*tmp;
//  }
//  init_p_eL2=p_eL2;
//
...
//    /* Note that p and e have been updated at a previous iteration */
///*     if (logfp) { */
///*       fprintf(logfp, "    it = %3d  p[%d]=( ", k, m); */
///*       for (int i=0; i<m; i++) fprintf(logfp, "%16.12f ", p[i]); */
///*       fprintf(logfp, " )\n"); */
///*     } */
//
//    if(p_eL2<=eps3){ /* error is small */
